...
public class WallCheck : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        dist_front = r_hf.distance == 0 || r_hf.collider.tag == "Wall" ? 1 : r_hf.distance / DIST_CHECK;
        RaycastHit r_hr;
        Physics.Raycast(Get_Right(), out r_hr, DIST_CHECK, 1);
        dist_right = r_hr.distance == 0 || r_hr.collider.tag == "Wall" ? 1 : r_hr.distance / DIST_CHECK;
        RaycastHit r_hl;
        Physics.Raycast(Get_Left(), out r_hl, DIST_CHECK, 1);
        dist_left = r_hl.distance == 0 || r_hl.collider.tag == "Wall" ? 1 : r_hl.distance / DIST_CHECK;
    }
